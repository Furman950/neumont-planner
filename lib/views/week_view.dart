...
class WeekView extends AbstractView {
  ...
  List<Widget> buildAssignmentCards(List<GuiObject> assignments) {
    ...
    if (assignments.length > 4) {
      ...
      assignments.forEach((a) => {
            toReturn.add(new AssignmentCard(a, false, true, true, false, false,
  ...
  List<Widget> buildEventCards(List<GuiObject> events) {
    ...
    if (events.length > 4) {
      ...
      events.forEach((a) => toReturn.add(new CustomEventCard()));
    }
    return toReturn;
  }
  ...
  List<Widget> buildCourseCards(List<GuiObject> courses) {
    ...
    if (courses.length > 4) {
      ...
      courses.forEach((a) => toReturn
          .add(new CourseCard(a, false, false, true, true, true, false, true)));
    }
    return toReturn;
  }
  ...
  buildWidgetsForWeek(DateTime date) {
    ...
    buildAssignmentCards(getObjectsByDay(date, getSortedAssignments()))
        .forEach((f) => toBeReturned.add(f));
    buildEventCards(getObjectsByDay(date, getSortedEvents()))
        .forEach((f) => toBeReturned.add(f));
    buildCourseCards(getObjectsByDay(date, getSortedCourses()))
        .forEach((f) => toBeReturned.add(f));
    return toBeReturned;
  }
}
